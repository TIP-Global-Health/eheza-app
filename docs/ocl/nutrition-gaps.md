...
- **Supply options**: `NutritionSupplementType` (FortifiedPorridge / RUTF / Ongera / TherapeuticMilk), `DistributionNotice`
  - *RUTF lookup (recheck 2026-04-15)*: PIH has no generic RUTF concept. Two brand-specific candidates exist — **PIH:12472 Plumpy'Nut** (*"Ready to use therapeutic food (RUTF) made from peanut butter"*, `Misc / N/A`, FSN in en/fr/ht) and **PIH:9533 Nourimanba (RUTF)** (*"Peanut-butter-based ready to use therapeutic food (RUTF) found in Haiti"*, `Misc / N/A`, FSN in ht/fr/en). Neither is a clean SAME-AS for a generic Rwandan RUTF supply concept; reviewer call required.
  - No matches for F-100, F-75, "therapeutic milk", "fortified blended flour", "FBF", "Ongera", or generic "micronutrient powder" in PIH.
