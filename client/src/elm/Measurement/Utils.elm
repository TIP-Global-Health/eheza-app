...
{-| A measurement, kept only while it is inside its range.

Used to build the task lists, so that a measurement outside its range counts as
a task still to do: Save stays disabled and the tasks counter goes down. The
counter doesn't say which measurement is at fault, only that one of them is.
...
measurementInRange isOutside value =
    if isOutside value then
        Nothing
