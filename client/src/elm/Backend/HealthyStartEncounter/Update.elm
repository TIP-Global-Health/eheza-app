...
update currentDate nurseId healthCenterId encounterId maybeEncounter msg model =
    case msg of
        ...
        HandleUpdatedHealthyStartEncounter data ->
            ( { model | updateHealthyStartEncounter = data }
            , Cmd.none
        ...
        --     )
        --
        -- SaveHealthyStartPhoto personId valueId value ->
        --     ( { model | saveHealthyStartPhoto = Loading }
        --     , saveMeasurementCmd currentDate encounterId personId nurseId healthCenterId valueId value prenatalPhotoEndpoint HandleSavedHealthyStartPhoto
        --     , []
        --     )
        --
        -- HandleSavedHealthyStartPhoto data ->
        --     ( { model | saveHealthyStartPhoto = data }
        --     , Cmd.none
        --     , triggerRollbarOnFailure data
        --     )
        --
...
updateEncounter :
    ...
    -> HealthyStartEncounterId
    -> Maybe HealthyStartEncounter
    -> (HealthyStartEncounter -> HealthyStartEncounter)
    -> Model
    -> ( Model, Cmd Msg, List App.Model.Msg )
updateEncounter currentDate encounterId maybeEncounter updateFunc model =
    maybeEncounter
        |> unwrap ( model, Cmd.none, [] )
            (\encounter ->
                ( { model | updateHealthyStartEncounter = Loading }
                , updateFunc encounter
                    |> sw.patchFull healthyStartEncounterEndpoint encounterId
                    |> withoutDecoder
                    |> toCmd (RemoteData.fromResult >> HandleUpdatedHealthyStartEncounter)
                , []
                )
            )
