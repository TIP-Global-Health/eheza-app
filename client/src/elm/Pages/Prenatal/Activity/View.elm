...
view language currentDate zscores site features id isChw activity db model =
    ...
    in
    viewWebData language (viewHeaderAndContent language currentDate zscores site features id isChw activity model) identity assembled


viewHeaderAndContent : Language -> NominalDate -> ZScore.Model.Model -> Site -> EverySet SiteFeature -> PrenatalEncounterId -> Bool -> PrenatalActivity -> Model -> AssembledData -> Html Msg
viewHeaderAndContent language currentDate zscores site features id isChw activity model assembled =
...
viewContent : Language -> NominalDate -> ZScore.Model.Model -> Site -> EverySet SiteFeature -> Bool -> PrenatalActivity -> Model -> AssembledData -> Html Msg
viewContent language currentDate zscores site features isChw activity model assembled =
    div [ class "ui unstackable items" ] <|
        viewMotherAndMeasurements language currentDate isChw assembled (Just ( model.showAlertsDialog, SetAlertsDialogState ))
            ++ viewActivity language currentDate zscores site features isChw activity assembled model
...
viewActivity : Language -> NominalDate -> ZScore.Model.Model -> Site -> EverySet SiteFeature -> Bool -> PrenatalActivity -> AssembledData -> Model -> List (Html Msg)
viewActivity language currentDate zscores site features isChw activity assembled model =
    case activity of
        PregnancyDating ->
...
viewNutritionAssessmentFormWithGWGIndicator language currentDate zscores site isHealthyStart assembled form previouslyMeasuredHeight prePregnancyWeight =
    let
        ...
        weightPreviousValueWithDate =
            resolvePreviousValueWithDate assembled .nutrition .weight

        weightPreviousValue =
            Maybe.map (Tuple.second >> weightValueFunc) weightPreviousValueWithDate
                ...
                let
                    bookingWeight =
                        resolveBookingWeight assembled
                            |> Maybe.map weightValueFunc
                in
                Maybe.Extra.or bookingWeight form.weight
                    |> calculateBmi form.height
        ...
        gwgData =
            if isHealthyStart then
                Maybe.Extra.andThen4
                    (\prePregnancyClassification previousWeight previousWeightDate currentWeight ->
                        resolveGWGClassificationForHealthyStart currentDate prePregnancyClassification previousWeight previousWeightDate currentWeight assembled
                            |> Maybe.map
                                (\classification ->
                                    let
                                        color =
                                            if classification == GWGInadequate then
                                                "red"
                                    ...
                                    , classification /= GWGInadequate
                                    )
                                )
                    )
                    baselineClassification
                    weightPreviousValue
                    (Maybe.map Tuple.first weightPreviousValueWithDate)
                    form.weight
