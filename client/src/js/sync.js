...
(function() {
      ...
      return dbSync.open().then(function() {
        return shardsToSync().then(function(shards) {
          var actions = shards.map(function(shard) {
            ...
            return uploadSingleShard(shard, credentials).then(
              function() {
                return downloadSingleShard(shard,
                  credentials);
              });
          });

          return Promise.all(actions).catch(function(err) {
            if ((err.tag === BadResponse) && (err.status ===
                401)) {
              return tryRefreshToken(credentials).catch(
                function() {
  ...
  function downloadSingleShard(shard, credentials) {
    return getLastVid(shard.uuid).then(function(baseRevision) {
      var url = getSyncUrl(shard, credentials) + '&base_revision=' +
        baseRevision;
  ...
  function getLastVid(shardUuid) {
    ...
    var collection = dbSync.shards.where('[shard+vid]').between(
      [shardUuid, Dexie.minKey], [shardUuid, Dexie.maxKey]
    );
  ...
  function sendToBackend(shardUuid, url, uploadUrl) {
    ...
    return countQuery.count().catch(formatDatabaseError).then(function(
      remaining) {
      if (remaining === 0) {
        var status = {
        ...
        return query.toArray().catch(formatDatabaseError).then(function(
          changes) {
          var status = {
            remaining: remaining,
          ...
          return dbSync.syncMetadata.update(shardUuid, {
            upload: status
          }).catch(formatDatabaseError).then(sendSyncData).then(
            function() {
              return uploadImages(table, changes, uploadUrl).then(
                function() {
                  return fetchWithTimeout(url, {
                    ...
                    if (response.ok) {
                      ...
                      return table.bulkDelete(ids).catch(
                        formatDatabaseError).then(function() {
                        return Promise.resolve(status);
                      });
                    } else {
                      return Promise.reject({
  ...
  function uploadImages(table, changes, uploadUrl) {
    return changes.reduce(function(previous, change) {
      return previous.then(function() {
        return uploadImageField(table, change, uploadUrl, 'avatar').then(
          function() {
            return uploadImageField(table, change, uploadUrl,
              'photo');
          });
      });
    }, Promise.resolve());
    ...
    if (change.data.hasOwnProperty(field)) {
      if (parseInt(change.data[field])) {
        ...
        return caches.open(photosUploadCache).then(function(cache) {
          return cache.match(change.data[field]).then(function(
            cachedResponse) {
            if (cachedResponse) {
              return cachedResponse.blob().then(function(blob) {
                ...
                return fetchWithTimeout(request, {}, imageTimeout)
                  .catch(function(err) {
                    return Promise.reject({
                      tag: NetworkError,
                      message: err.message,
                    ...
                    if (response.ok) {
                      return response.json().catch(function(err) {
                        ...
                        change.data[field] = parseInt(json.data[
                          0].id);
    ...
    return fetchWithTimeout(url, {}, dataTimeout).catch(function(err) {
      ...
      if (response.ok) {
        return response.json().catch(function(err) {
          ...
          var table = shardUuid === nodesUuid ? dbSync.nodes : dbSync
            .shards;
          ...
          return json.data.batch.reduce(function(previous, item) {
            return previous.then(function() {
              return formatNode(table, item, shardUuid).then(
                function(formatted) {
          ...
          if (node[field]) {
            ...
            return cachePhotoUrl(node[field]).then(function() {
              ...
              if (existing[field] && node[field] !== existing[field]) {
                return deleteCachedPhotoUrl(existing[field]).then(
                  function() {
      ...
      return cache.match(url).then(function(cached) {
        if (cached) {
          ...
          return getCredentials().then(function(credentials) {
            ...
            fetchUrl.searchParams.set('access_token', credentials
              .access_token);
            ...
            return fetchWithTimeout(req, {}, imageTimeout).then(
              function(response) {
                if (!response.ok) {
                  throw new TypeError('Response status ' +
                    response.status + ' fetching ' + url);
                }
  ...
  function deleteCachedPhotoUrl(url) {
    return caches.open(photosDownloadCache).then(function(cache) {
      return cache.delete(url).then(function() {
        return caches.open(photosUploadCache).then(function(
          uploadCache) {
  ...
  function tryRefreshToken(credentials) {
    var refreshUrl = credentials.backend_url + '/api/refresh-token/' +
