...
(function() {
  ...
  function deleteNode(url, type, uuid) {
    return dbSync.open().catch(databaseError).then(function() {
      if (type === 'syncmetadata') {
        ...
        return dbSync.syncMetadata.delete(uuid).catch(
          databaseError).then(
  ...
  function putNode(request, type, uuid) {
    return dbSync.open().catch(databaseError).then(function() {
      return getTableForType(type).then(function(table) {
        return request.json().catch(jsonError).then(
          function(json) {
            json.uuid = uuid;
            json.type = type;

            return table.put(json).catch(
              databaseError).then(
              function() {
  ...
  function view(url, type, uuid) {
    return dbSync.open().catch(databaseError).then(function() {
      if (type === 'child-measurements') {
        ...
        return getTableForType(type).then(function(table) {
          return table.get(uuid).catch(
            databaseError).then(function(
            node) {
  ...
  function index(url, type) {
    ...
    return dbSync.open().catch(databaseError).then(function() {
      ...
      return getTableForType(type).then(function(table) {
        ...
        if (type === 'person') {
          var nameContains = params.get(
            'name_contains');
          if (nameContains) {
            modifyQuery = modifyQuery.then(
              function() {
                query = table.where(
  ...
  function determineShard(node) {
    if (node.session) {
      return dbSync.nodes.get(node.session).then(function(session) {
        return dbSync.nodes.get(session.clinic).then(
          function(clinic) {
            if (clinic) {
  ...
  function makeUuid() {
    ...
    return caches.open(configCache).then(function(cache) {
      return cache.match(deviceUuidUrl).then(function(
        response) {
        if (response) {
          ...
          var cachedRequest = new Request(
            deviceUuidUrl, {
              method: 'GET'
            });

          return cache.put(cachedRequest,
            cachedResponse).then(
            function() {
      ...
      return Promise.resolve(kelektivUuid.v5(timestamp,
        deviceUuid));
    });
  }
