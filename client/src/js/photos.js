...
(function() {
  self.addEventListener('fetch', function(event) {
    ...
    if ((event.request.method === 'GET') && photosDownloadUrlRegex.test(
        event.request.url)) {
    ...
    if ((event.request.method === 'GET') && photosUploadUrlRegex.test(
        event.request.url)) {
    ...
    if ((event.request.method === 'POST') && photosUploadUrlRegex.test(
        event.request.url)) {
      var response = caches.open(photosUploadCache).then(function(cache) {
        return cache.keys().then(function(keys) {
          ...
          var index = 1;
          while (true) {
            var url = (new URL("cache-upload/images/" + index,
          ...
          return event.request.formData().then(function(formData) {
            ...
            return cache.put(eventualRequest,
              eventualResponse).then(function() {
