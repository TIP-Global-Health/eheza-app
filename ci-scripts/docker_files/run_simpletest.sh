...
cd "$ROOT_DIR"/server
php ./www/scripts/run-tests.sh -d memory_limit=256M --php "$(which php)" --concurrency 4 --verbose --color --url http://server.local Hedley 2>&1 | tee /tmp/simpletest-result.txt
