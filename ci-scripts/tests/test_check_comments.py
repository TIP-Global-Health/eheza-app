...
class CodeComments(unittest.TestCase):
    ...
    def test_a_scss_comment_counts(self):
        source = "// one two\n.a { color: red; }\n/* three */\n"
        self.assertEqual(sizes("a.scss", source), [2, 1])

    def test_an_unknown_file_type_has_no_comments(self):
        self.assertEqual(sizes("a.docx", "# one two\n"), [])

    def test_a_vendored_file_is_not_read(self):
        self.assertEqual(sizes("client/src/elm/AssocList.elm", "-- one two\n"), [])

    def test_an_elm_test_module_is_not_read(self):
        path = "client/src/elm/Pages/NCD/Test.elm"
        self.assertEqual(sizes(path, "-- one two\n"), [])

    def test_a_server_test_is_not_read(self):
        path = "server/hedley/modules/custom/hedley_stats/tests/run.php"
        self.assertEqual(sizes(path, "// one two\n"), [])
