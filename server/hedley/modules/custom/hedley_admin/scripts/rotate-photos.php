...
$base_query = db_select('field_data_field_photo', 'photos')
  ->fields('photos', ['entity_id'])
  ->condition('field_photo_width', 800)
  ->condition('field_photo_height', 600);

$count_query = clone $base_query;
$count = $count_query->execute()->rowCount();

if ($count == 0) {
  drush_print("There are no photos in DB.");
  exit;
}

drush_print("Located $count photos that require rotation");
