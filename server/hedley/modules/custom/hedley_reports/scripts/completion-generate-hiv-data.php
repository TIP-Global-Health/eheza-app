 ...
 *
 * Execution: drush scr
 *   profiles/hedley/modules/custom/hedley_reports/scripts/completion-generate-hiv-data.php.
 */
...
if ($count == 0) {
  drush_print("There are no nodes of type $type for hiv encounters in DB.");
  exit;
}

$total = 0;
drush_print("$count nodes of type $type for hiv encounters located.");
...
while (TRUE) {
  ...
  foreach ($nodes as $node) {
    hedley_reports_generate_completion_data_for_hiv($node, $exclude_set);
    $total++;
