...
## Open right now — updated 2026-09-21

⛔⛔ **THE RELEASE GATE IS RED, AND HAS BEEN SINCE 2026-09-16 — read this before cutting #2128.** (✅ PR #2271 MERGED 2026-09-22 as `df17681aa` — the gate is fixed; the next `develop` build shows it.)
All three e2e jobs fail on `develop` on every revision since `d51fbef56` (the PR #2238 translations-fold merge): `d51fbef56`, `26cd53901`, `b2f41d5e5`, `04125382a`. Cause: the fold changed English strings and four e2e literals still expect the old ones (`"TDF+3TC"` vs `'TDF + 3TC'`, `"1 day"` vs `getByText('1 Day', {exact:true})`, `"Referred to health center"` vs `selectOption({label:'Referred to Health Center'})`). **Recorded as [[B-395]], tier 2 — ✅ fix is PR #2271 (issue #2270, 2026-09-22), awaiting merge; `develop` stays red until it lands, and every PR that merges `develop` in inherits the redness (PR #2262 did on 2026-09-22).** Nothing caught it because all five commits of #2238 carry `[ci skip]` and `develop` requires no status checks ([[B-397]]). ⚠ Open PR branches were cut before the fold, so their runs are green — the redness exists only on `develop`.

🔎 **R36 discovery COMPLETE 2026-09-22** — the last rows never re-read at R25+ depth (sub-updaters, NCDA cross-side, dashboard stats cross-side, person search/enrolment, expectActivity vs Completion), 5 units at 76e4d3e04, **9 new items B-424..B-432**: 0 tier 1, 0 tier 2, **five tier 3** — **B-424** Prenatal recurrent labs: the last result's save PATCHes the labs-results record twice and the stale second wins, so the next encounter asks for that test again (16 HIV-fingerprinted records live); **B-427** Child Scoreboard scorecard shows red X on months 0–5 of four rows the form never asks before six months; **B-428** aggregated NCDA % rows divide by every child under two; **B-429** aggregated immunisation ladder rebuilt from dates, 788 child-vaccine pairs disagree live; **B-430** nurse Clinics page mints a second group session while the HC is uploading (582 same-device duplicate clinic-days on ihangane). Tier 4: B-425, B-426, B-431, B-432. Amended B-368 (d), B-330. ⚠ Scouts died once on the Fable usage limit and were re-run on Opus. See `rounds.md` Round 36.

🔎 **R35 discovery COMPLETE 2026-09-21** — the oldest never-re-read rows, 5 units, **6 new items B-418..B-423**: ⭐ **two tier 1** — **B-421** NCD pregnancy test keeps its hidden result when changed to "not performed", so pregnancy status (and the ACE-inhibitor gate) can come from a withdrawn test (the B-320 fix reached only the universal forms; found by two scouts independently; 0 stale pregnancy records live, 1 stale NCD HIV record); **B-422** HIV Diagnostics hard-codes `testResultDirty = False`, so correcting Positive to "not run" stores Positive and never closes the participant (latent: no live record has a result). One tier 3 — **B-418** adult Patient Record Family Planning pane omits NCD family planning (746 records / 545 women on ihangane). Tier 4: B-419, B-420, B-423. Amended B-177, B-199, B-283 (its fix shape cannot work). See `rounds.md` Round 35.

🔎 **R34 discovery COMPLETE 2026-09-21** — the verification-and-delivery layer, 6 units, **25 new items B-393..B-417**: one tier 2 (B-395 above, ✅ PR #2271) and five tier 3 — **B-393** one e2e test has run in NO CI job since 2026-03-15 (`NCD` matches inside `NCDA`; ⚠ TH-05 is queued work written entirely about that test), **B-394** `healthy_start` is on in Rwanda production and enabled in no dev or CI environment, **B-396** `e2e_playwright_1`'s `&&` silently skips 25 of 65 tests whenever the first leg fails, **B-397** merge-with-no-CI is possible today, **B-413** admin lists and CSVs show soft-deleted content (159 deleted people live on ihangane). Amendments: B-331 re-confirmed live on all four sites (backend error reporting still dead, one premise corrected), B-269, B-248, B-144 (→ split B-404), B-145, B-157 (verdicts on all eight sub-items), B-380, B-330, B-387. See `rounds.md` Round 34.

✅ **PR #2269 MERGED 2026-09-22 (opened 2026-09-17) (B-259 + B-222, tier 4, issues #2267 / #2268)** — person form: entering the create or edit page resets the app-wide person save request, so a fresh form no longer opens with "Person has been saved" (or an old error); a person PATCH stores its real result, so a failed edit shows the backend error. Three Elm files; elm make, elm-review, elm-test green locally; no new test. Worktree released. **Review round 1 (medium, 2026-09-17): 2 findings posted inline** — children's address PATCHes share `postPerson` and navigate (hides the parent's failure); WhatsApp PATCH failure lingers for the contact form. Both fixed at the user's call in `cb2505cba`: child address saves use `InitiatorChildAddressUpdate`, and only `InitiatorEditForm` writes `postPerson` (Loading and reply) or navigates; threads replied and resolved, PR body updated. elm make, elm-review, elm-test green locally. CI on `cb2505cba`: all green except e2e_playwright_1, which failed on two flaky tests (B-379 switchUser, and new B-380 lemon/honey timeout — the same job passed on `48a18fb56`); retriggered with empty commit `b5d0c742d`. ✅ **CI fully green on `b5d0c742d`** (all 11 checks; e2e job 1 passed on rerun, confirming the two flakes). **Review round 2 (medium, 2026-09-17): 2 low findings posted inline** — entry reset drops the `Loading` double-save guard; a failed edit's `Failure` reaches the Acute Illness register-contact form. **User decided 2026-09-22: apply the first, won't-fix the second.** `49d71758e` keeps a `Loading` request on entry and clears only a finished one; both threads replied (fixed / won't-fix: contact tracing is obsolete, zero live nodes, and a failed create already reached that form). elm make, elm-review, comment check green locally; ✅ **CI fully green on `49d71758e`** (all 11 checks; the branch predates the fold, so its e2e are unaffected by B-395). Worktree released. **Awaiting merge.**

🟡 **PR #2291 open 2026-09-24 (B-318, tier 2, issue #2290)** — Sync: the dashboard statistics GET carries the 60 s upload timeout (constant renamed `transferRequestTimeout`), so a stalled download fails and the next tick retries instead of wedging sync until reload. Two Elm files; elm make, elm-review, comment check green locally; not browser-run. Photo-lane twin recorded as [[B-435]] (tier 4). Worktree released. **Reviewed 2026-09-24 (medium): 1 finding, fixed in `2562c87a1`** — a failed stats download now moves on (authority marked `Error`), own 5-min timeout; thread answered.

🟡 **PR #2287 open 2026-09-24 (B-233 TB half, tier 2, issue #2286)** — Case Management: the TB-suspect entry is hidden once a TB encounter started on or after its date, and a tap reuses the open TB participant. Four Elm files, 4 elm-tests (2 fail with the filter stubbed); elm make, elm-test, elm-review, comment check green locally. HIV half split to B-434 (tier 4). Worktree released. **Review round 1 (medium): 2 findings posted inline** — same-day date check won't-fix (user; 1 of 198 live either way), duplicated open-participant lookup fixed in `0a60ed1c1` (`resolveOpenParticipant`, shared with the TB and HIV participant pages). Threads resolved. elm make, 3261 elm-tests, elm-review, comment check green locally. CI pending. ⚠ CI on `0a60ed1c1`: e2e_playwright_2 died in *Install and build client* on the Elm-compile 4-min no-output watchdog (no test ran); retriggered 2026-09-24 with empty commit `3e7e6c2c3` (user's call).

🟡 **PR #2289 open 2026-09-24 (B-216, tier 2, issue #2288)** — a birth-date change moves group enrolment/graduation dates only where the person is the child; an adult's correction no longer rewrites their children's. One PHP file + scenario 4 in `HedleyStatsCalculation` (before B-248's early return). Test `cc9771a82` + fix `2757fde08` (user: push the fix without waiting). Fail-first proven anyway: simpletest on `cc9771a82` failed exactly the mother-side assertion (build 31211). Worktree released. **Reviewed 2026-09-24 (medium): no findings.** ✅ **CI fully green on `2757fde08` (all 11 checks; new assertions pass in build 31222).** Awaiting merge. Live damage: ihangane 560, vhw 22, none current; backfill not done (user: code fix only).

🟡 **PR #2285 open 2026-09-23 (B-241, tier 3, issue #2284)** — Prenatal: the continued-care hypertension block (previously diagnosed patients) shows at the initial phase only, per issue #256; at the recurrent phase it no longer locks Save. One source line + 2 elm-tests (discriminating). Worktree released. ✅ **CI fully green on `d5b292767` (all 11 checks).** **Reviewed 2026-09-23 (medium): no findings.** Awaiting merge.

🅿 **B-325 PARKED 2026-09-23 (tier 2)** — user: "park", after the live frequency estimate (~1–3 lost uploads/year on ihangane). Tier 2 now has no READY items.

✅ **B-341 DONE 2026-09-23 (tier 2, no code)** — release PR #2128's post-deploy steps now run `restore-wiped-photos.php` (dry run, then real, after `drush fra`; ~3,213 persons) and remind to re-run the B-290 beverages cleanup later; decision recorded on issue #2151, which stays open until the script has run on the sites.

🟡 **PR #2283 open 2026-09-23 (B-422, tier 1, issue #2282)** — HIV program Diagnostics: the result's dirty flag survives hydration; a result is saved only with a test run; the close-as-not-diagnosed check reads the saved value; repeated taps on "diagnosed positive" / "test run" leave the form alone. Four Elm files incl. the first HIV test file (7 cases through the real `update`). Worktree released. ✅ **CI fully green on `721c5cb41` (all 11 checks).** **Reviewed 2026-09-23 (medium): no findings.** Awaiting merge.

🟡 **PR #2279 open 2026-09-22 (B-433, tier 4, issue #2278)**: Prenatal HIV / Partner HIV / Hepatitis B bool inputs now update the form merged with the saved test, so B-320's repeat-tap guard holds for a reopened test. Three Elm files, 4 tests through the real `update` (3 fail on the raw form). Worktree released. **Reviewed 2026-09-22 (medium): no findings.** ✅ **CI fully green on `68041b65a` (all 11 checks)** — awaiting merge. ⚠ Conflicted with `develop` 2026-09-23 (B-421's NCD rapid-test helpers and B-392's test group landed beside it); resolved by merging `origin/develop` (`6f1dafce0`): export list = develop's + `knownAsPositiveUpdate`, both test groups kept. elm make, 3259 elm-tests, elm-review green locally. CI on `6f1dafce0` red on `lint_elm` only: the joined test groups needed elm-format's two blank lines; fixed in `c126ae006`, whole tree validates. ✅ **CI fully green on `c126ae006`** (all 11 checks).

✅ **PR #2275 MERGED 2026-09-23 as `0cb0bd929` (opened 2026-09-22) (B-421, tier 1, issue #2274)** — NCD Pregnancy/HIV test forms clear the result, as dirty, when the run is withdrawn ("performed" → No, or known as positive); the two NCD result setters mark it dirty; `patientIsPregnantAtEncounter` takes a result only from a run that produced it (covers saved records and old-build devices). Six Elm files (4 source + 2 test), 12 elm-tests, 7 fail with the fix stubbed; elm make / elm-review / comment check green locally. **Reviewed 2026-09-22 (medium): 1 low finding (repeat tap on the bool inputs now wiped an entered result), user chose the guard — fixed in `428ff1898` (top-level `rdtTestPerformedUpdate` / `rdtKnownAsPositiveUpdate`, guarded like the universal forms), thread resolved.** **Review round 2 (medium): 3 findings (guards read the raw form; withdrawn HIV run kept its signs), all fixed in `f6f5a0656` at the user's call, threads resolved; Prenatal's twin filed as B-433 (tier 4). Review round 3: no findings.** ⚠ CI runs 1–3 red on `e2e_playwright_1` only, every time on the B-379 lab-tech race (3, 2, 1 hits) + one B-380 hit; ✅ **run 5 on `f6f5a0656` FULLY GREEN (all 11 checks)** — awaiting merge — if it is red on B-379 again the merge is the user's call, or build B-379's one-line fix first. Worktree released. Sibling B-422 (HIV Diagnostics `testResultDirty = False`) is still READY.

✅ **PR #2273 MERGED 2026-09-23 as `35fa48642` (opened 2026-09-22) (B-392, tier 1, issue #2272)** — prenatal report-back guard: the diabetes / rhesus-negative "already known" check now asks the ORIGINATING encounter (new `diagnosisKnownAnyOf` = history ∪ own ∪ `pastDiagnoses`) instead of the older encounter, whose history can never hold a later visit. Three Elm files; 5 elm-tests through the `assembledFromDb` harness, discrimination run done (history-only reader fails the two new legs). **Reviewed 2026-09-22 (medium): 1 finding (medium), posted inline, fixed at the user's call in `6e85fff17`** — the reader ignored a PREVIOUS visit's `pastDiagnoses`, so with two pending tests of one kind the second late result re-referred; the reader now applies one rule (own ∪ past diagnoses) to the encounter and every earlier visit, sixth test case fails on the previous reader; thread replied. CI on `a70ab257f`: 10/11 green, e2e job 1 red on the B-380 lemon/honey flake only; CI on `6e85fff17`: 10/11 green, e2e job 1 red on the B-379 `switchUser` flake (lab-tech spec, before any clinical step) — retriggered with an empty commit `bd54c88e2`. ✅ **CI fully green on `bd54c88e2`** (all 11 checks). Worktree released. **Awaiting merge.** ⚠ The defect it fixes came in with B-364 and is NOT deployed yet — this should land before release #2128 is cut.

✅ **PR #2277 MERGED 2026-09-23 as `f08f302b3` (opened 2026-09-22) (B-380, tier 4, issue #2276)** — e2e acute illness: the save helper waits for the saved form to leave and then for whichever page the app moves to (encounter page, or the Next Steps / Laboratory activity it opens itself once the diagnosis is made). Mechanism read from the failing run's screenshot: the save worked, the app was already on Next Steps with the assessment popup — the R34 "dropped click" lead is refuted. One helper file. Not run locally; CI is the first run (and the flake is intermittent, so one green run is weak evidence). Worktree released. **Reviewed 2026-09-22 (medium): no findings** (the reviewer checked every self-navigation the app performs after a diagnosis-completing save lands on a differently classed wrapper, so the hidden-wait cannot hang). ✅ **CI fully green on `1fd826d2c`** (all 11 checks; the lemon/honey test passed in e2e job 1 — one green run, weak evidence for an intermittent flake). **Awaiting merge.**

🟡 **PR #2281 open 2026-09-23 (B-379, tier 4, issue #2280)** — e2e lab tech: `switchUser` opens `about:blank` before clearing the site's storage, so the app cannot change its URL mid-login and interrupt `pairDevice`'s `goto('/')`. One spec file, three lines. Not run locally; intermittent, so one green run is weak evidence. Worktree released. **Reviewed 2026-09-23 (medium): no findings** (storage clear targets the origin so the open page does not matter; no `beforeunload`; `setupDevice`'s single Back step never reaches `about:blank`). ✅ **CI fully green on `6d7436260`** (all 11 checks; all four lab-tech tests passed in e2e job 1 — one run, weak evidence for an intermittent race). **Awaiting merge.**

✅ **PR #2271 MERGED 2026-09-22 (opened 2026-09-22) (B-395, tier 2, issue #2270)** — e2e suite: four exact-match literals now expect the strings the translations fold settled on (`TDF+3TC` in the prenatal and lab-tech specs, `1 day` in the group-session helper, `Referred to health center` in the acute-illness CHW spec). Sweep of every fold-removed English string against `client/e2e` found nothing else stale. Not run locally. ✅ **CI fully green on `38d638cbe`** (all 11 checks; the three e2e jobs that were red on `develop` pass — the first green e2e run on the fold). Worktree released. **Reviewed 2026-09-22 (medium): no findings** (the reviewer also confirmed `helpers/nutrition.ts` `hasText: '1 Day'` still matches, since `hasText` strings are case-insensitive). ⛔ Unblocks the release gate; merge before anything that needs green e2e on `develop`.

✅ **PR #2266 MERGED 2026-09-22 (opened 2026-09-17) (B-317, tier 2, issue #2265)** — sync download: a reply is applied only when it belongs to the request in flight (each authority/general GET now carries its issue time) and to the HC the cycle is on (a reply or save ack for an HC that is no longer current is dropped, and the current one refetched next tick — this also closes the R29 add/remove-HC amendment); the stats reply updates the model's list instead of writing back its request-time copy; **Reviewed 2026-09-17 (medium): 2 findings, both fixed in `f2dd38662`** — the 30 s `withTimeout` + drop-by-time could stall a batch that always takes >30 s, and the UUID-only identity let a removed-and-re-added HC accept the in-flight batch. Identity is now authority + base revision (`isCurrentAuthorityRequest`), a matching reply lands however late, the HTTP timeout is gone, the general lane carries its base. 9 elm-tests, 6 fail with the guards stubbed. Threads answered and resolved. Worktree released. ✅ **CI fully green on `f2dd38662` (all 11 checks incl. simpletest + 3 e2e).** ⭐ **Verified on the real local stack 2026-09-17, single HC: fresh full download equals the server; a 70 s first batch lands and its 30 s duplicate is discarded; reload mid-download resumes from the persisted cursor; on the imported LIVE DB, general 4 batches and Mushongi 11 batches (5,235 records) land with every base = previous top revision** (recipe in memory `verify-sync-lanes-in-browser`). Failure-branch tests added in `2bd8bde25`, ✅ **CI fully green (all 11 checks)**. Awaiting merge.
...
✅ **PR #2264 MERGED 2026-09-22 (opened 2026-09-17) (B-375, tier 3, issue #2263)** — patient merge: the acute-illness step's two participant lookups now read the mapping constant (`acute-illness`) instead of the literal `acute_illness` that never matched, so the duplicate's acute-illness participants move to the original and the delete cascade no longer soft-deletes the encounters and measurements just moved. Consolidation test gains case 19 (participant + encounter + vitals live on the original after a full merge); CI is its first simpletest run — fail-first checked locally via drush in a rolled-back transaction. Worktree released. **Reviewed 2026-09-17 (medium): no findings.** ❌ First CI run on `28a13340e` red: simpletest threw in the new case's fixture (encounter created empty, device shard hook reads the participant's person on first save — reproduced locally), fixed in `deaacacdf`; e2e_playwright_1 failed in the acute-illness lemon/honey report test (timeout in `saveActivity`), not touched by this PR. ✅ **CI fully green on `deaacacdf`** (all 11 checks); the simpletest log shows the five acute-illness merge assertions passing, and the e2e test passed on rerun (flake). **Awaiting merge.**

✅ **PR #2262 MERGED 2026-09-22 (opened 2026-09-17) (B-311, tier 2, issue #2261)** — aggregated NCDA scoreboard: the selection floor is the fixed date 2021-11-01 (two years before the app's Nov 2023 first month) instead of three years before the request, so historical columns keep the children who were under two at the time. One PHP line plus a comment; the endpoint test gained an in-range/out-of-range child pair, CI is its first run. Worktree released. ✅ **CI fully green on `18099a952` (all 11 checks incl. simpletest).** **Reviewed 2026-09-17 (medium): no findings.** ⚠ Conflicted with `develop` on the endpoint test (both sides added a check after `checkPrevalencePercentages()`); resolved 2026-09-22 by merging `origin/develop` (`a3d6711db`, both checks kept). That merge pulled the translations fold in, so its CI is now red on the three e2e jobs exactly as `develop` is ([[B-395]], PR #2271) — simpletest and all lints still pass. #2271 merged 2026-09-22; `develop` merged in again (`06019342c`). ✅ **CI fully green on `06019342c` (all 11 checks incl. the three e2e).** **Awaiting merge.**
...
✅ **PR #2251 MERGED 2026-09-16 (opened 2026-09-15) (B-359, tier 3, issue #2250)** — Acute Illness progress report: lemon juice/honey is shown in Actions Taken only when the medication distribution records it (was printed for every cold-and-cough visit); the "given?" check in that function is now one `medicinePrescribed` shared by all six medicines. New nurse e2e case (No → absent, edited to Yes → shown), in CI job 1, **not run locally**. Worktree released after push. **Reviewed 2026-09-15 (medium): no findings.** CI runs 1–2 red on the new e2e case only (test route: Next Steps opens by itself, with an assessment popup; re-saving opens the report). Fixed and **run locally 2026-09-16: fails on develop, passes on the fix, acute-illness specs 7/7** (`95bfd75a4`). ✅ **CI fully green on `95bfd75a4`** (all 10 checks). Awaiting merge.

✅ **PR #2253 MERGED 2026-09-16 (opened 2026-09-15) (B-344, tier 1, issue #2252)** — Prenatal HIV (lab tech): `hivProgramAtHC` is three-valued (Yes / No / not yet answered, via `hivSignsAnswered`); the HIV medication set needs the answered No, the ARV referrals (initial, recurrent, previous-encounter reader) need the answered Yes; `hedley_reports` medication-expected copy skips the HIV set on `pending-input`. 3 elm-tests (pending case fails on develop). **New e2e in `lab-tech-encounter.spec.ts` — not run locally, CI is its first run** (title avoids the grep-invert words, so it runs in e2e job 1). **Review round 1 (medium, 2026-09-15): 6 findings, all posted inline and fixed in `22261e498`, threads resolved** — two were real (the e2e called `completeRecurrentNextSteps` from inside the activity page, and used the initial-phase referral icon name on the recurrent page; CI's first e2e run failed exactly there, at spec line 706, after the medication-tab-absent assertion had PASSED on the real app), plus fixture/helper dedup, a flat `case` reader with uniform `== Just True/False` callers, and the no-signs clause in the doc comment. Phase 1 dedup of the lab-tech spec deliberately NOT done (user: suite-wide fixture, not this PR). Worktree released. **CI FULLY GREEN on `123d83204`** (all 10 checks; the earlier `22261e498` run was red only on the `switchUser` navigation flake in another lab-tech test, filed B-379, tier 4 — the new B-344 e2e passed in that run too). **Awaiting merge.** B-342 🅿 PARKED the same day (measured rare on ihangane).

✅ **PR #2249 MERGED 2026-09-16 (opened 2026-09-15) (B-365, tier 1, issue #2248)** — Prenatal: Medical History, both Obstetric History steps, Obstetrical Exam, Nutrition and Ultrasound saves now re-run the nurse assessment (six new `HandleRevisions` branches through a shared `assessPrenatalOnRevision`; nine identical existing branches moved onto it). New e2e case in the nurse initial-encounter spec — not run locally, CI is its first run. Worktree released. **Reviewed 2026-09-15 (medium): no findings.** ✅ **CI fully green on `b26db8b61` (all 10 checks incl. simpletest + 3 e2e)** — the first three runs died on GitHub package downloads during a GitHub incident; `e2e_playwright_1` ran 11 tests = the job-1 specs' 6+2+3, so the new case ran and passed. **Awaiting merge.**

✅ **PR #2241 MERGED 2026-09-16 (opened 2026-09-14) (B-339, tier 1, issue #2240)** — NCD history cut to encounters that started before the one assembled (`generateAssembledData`), so a lab result entered after the next visit no longer restages the older encounter from the later one. Keeps the #2216-introduced stage half out of release #2128 if merged before it is cut. CI green, reviewed (1 finding — report from an older encounter shows it as of that encounter; user chose to keep that), thread resolved — **awaiting merge**. ⚠ B-364's presentation note calls #2241's same-day edge "not rare" from a raw pair count; for NCD 210 of 217 pairs are pre-guard double-tap duplicates (see B-339) — re-check the Prenatal 4,127 the same way before designing around it. ✅ Re-checked 2026-09-14: same shape (4,693 same-device ≤60 s duplicates); #2243 now uses the strict cut too. B-364 (Prenatal twin) is next, as a separate PR.
...
✅ **PR #2243 MERGED 2026-09-16 (opened 2026-09-14) (B-364, tier 1, issue #2242)** — the Prenatal twin: `generatePreviousMeasurements` keeps only encounters that started on an EARLIER day (strict `LT`, same as #2241; the id check was dropped as redundant). CI fully green on `d6c7eb48d` (all 10 checks incl. simpletest + 3 e2e). **Reviewed 2026-09-14 — 1 finding (report from an older encounter hides later visits), user: *"strict cut, keep the report as at #2241"* — thread answered and resolved.** 6 elm-test cases: 5 fail on develop, 2 fail on the old `/= GT` cut. Covers B-363(d). **Awaiting merge.**

✅ **PR #2245 MERGED 2026-09-16 (opened 2026-09-14) (B-343, tier 1, issue #2244)** — Prenatal HIV: the partner answers on the patient's own HIV test count only while the Partner HIV lab has no result (`partnerSignsByHIVTest`, the form's condition), in all four readers (diagnosis, report line, TDF3TC helper, HIV health education). User chose *"follow the form"*: a partner test pending at the lab counts as a result too. 6 of 9 new elm-tests fail with the gate removed. E2E extended in the existing nurse discordant-partnership test (`4e6e580da`) — fails on develop, passes on the fix, run locally. Review round 1: no bugs, 1 style finding (test fixture duplication) — fixed in `c654c32f9`, thread resolved. `4e6e580da` fully green (the lab-tech `pairDevice` navigation failure on the first commit was a flake). Worktree released. ✅ **CI fully green on `c654c32f9` — awaiting merge.**
...
**#2210 (B-323, tier 1, issue #2209) opened 2026-09-07** — ⚠ **stacked on `B-237-discordant-partnership-phase`, so merge #2206 first**, then it rebases onto `develop` and its diff is four files alone. The lab-tech execution note (`TestNoteRunConfirmedByLabTech`) was omitted by **four** readers, not the two the entry named — the extra two are the progress report's discordant-couple line and the HIV health-education branch. Sites 2-4 are LIVE today on all 110 lab-tech-confirmed partner tests on ihangane (all negative): the nurse is re-asked for the partner status the lab already reported. 4 new elm-test cases, discrimination-run against the pre-fix sources. **Reviewed 2026-09-07 — two findings, both fixed in `ed0cd5216`, both threads answered.** ⭐ The first was a defect I had SEEN and waved off in the presentation ("it self-corrects"): the pending-input marker was read as "partner not on suppressed ARVs", and because recurrent Next Steps is **not** gated on the follow-ups being answered, PrEP could be distributed in that window and the prescription survives the diagnosis withdrawing. ⛔ **Lesson: "it self-corrects on the next save" is not a safety argument until you check what the window lets a user WRITE.** The second renamed `completeLabResultsAsLabTech` → `completeLabResults` (it was never lab-tech-specific) and added a real two-role e2e case in `lab-tech-encounter.spec.ts` on the seeded lab-tech PIN `3333`. **Round 2 (same day) found I had fixed ONE of three readers of the partner signs** — the progress report rendered "Discordant Couple: Partner NOT taking ARVs" from the pending marker, on the very page the lab tech lands on after saving. Fixed in `47086609e`: the sentinel test is now `hivSignsAnswered` in `Measurement/Utils.elm`, and the report line moved into `resolveDiscordantCoupleStatus` in `Pages/Prenatal/Utils.elm` — ⭐ **because `ProgressReport/View.elm` is NOT in the elm-test module graph while `Pages/Prenatal/Utils.elm` is; moving logic to the Utils module already in the graph is the way to make view logic testable here.** ⛔ **Lesson: after fixing one reader of a value, fix the LIST of readers.** ⛔⛔ **And the lab-tech e2e case initially RAN IN NO CI JOB** — `e2e_playwright_1`'s `--grep-invert` list contains "HIV" and the test's title did too; the file is in no other job's list. Green e2e, zero coverage. Folded into the existing `lab-tech-encounter.spec.ts` test in `9f2255346` (title carries no "HIV", and no second full encounter added to jobs already at 30–32 min). ⚠ `playwright test <token>` matches the **absolute** path, so measuring the partition from a worktree named `…-partner-hiv-labtech-note` reports every spec as matching `hiv` — measure from a neutrally-named directory. Backlog **G-04** is the standing guard for this class. **Round 3 (2026-09-07): three findings.** Its headline is the MIRROR of round 2 — the pending guard means the diagnosis is absent while the nurse may already have saved Recurrent Next Steps, and `nextStepsTaskCompleted` accepts the "nothing to give" marker for ever, so the later TDF3TC row is never shown ⇒ **filed as B-336 (tier 1), deliberately not fixed in #2210** since the shared completion rule serves malaria / syphilis / hypertension / anemia too. The e2e anchor and the progress-report assertions the user asked for are in `4184bf276`. ⛔ **Correction carried forward: the LAB TECH never saw the false discordant line** — for a lab tech `labResultsMode` defaults to the lab-results pane and no diagnosis panes render; it is the NURSE who sees it before completing the follow-ups.

**#2214 (B-336, tier 1, issue #2213) opened 2026-09-07** — ⚠ **stacked on `B-320-known-positive-stale-result` (#2212), which is itself stacked on `B-323-partner-hiv-labtech-note` (#2210): merge #2210, then #2212, then this.** The user chose the stack over merging the parents first, because both parents already edit `client/e2e/helpers/prenatal.ts` and `client/e2e/prenatal-encounter-nurse.spec.ts`. Elm side is clean of both: neither parent touches `Pages/Prenatal/RecurrentActivity/{Utils,Test}.elm`, and their `Pages/Prenatal/Utils.elm` hunks are nowhere near the completion predicate.
⚠ **#2210 does not have a defect of its own, but it OPENS A SECOND DOOR into B-336** — its `hivSignsAnswered` guard delays the discordant-partnership diagnosis past a possible Recurrent Next Steps save. B-336's class is live on `develop` regardless, through the ungated BP recheck → hypertension → `DiagnosisHighRiskOfPreeclampsiaRecurrentPhase` → Aspirin route. That is a reason to land the three close together, not to hold #2210.
**Reviewed 2026-09-07 — zero correctness findings; both quality findings fixed in `f2ad60632` and both threads answered.** ⚠ Its red `e2e_playwright_1` is INHERITED from #2212 (that PR's own new test fails on its own branch too, build 29851 — `openActivity` in `client/e2e/helpers/common.ts` never switches to the Completed tab, so an activity card that has moved there is unreachable); a parallel session is fixing it. ⛔ Do not chase it from this PR.
✅ **E2E WRITTEN AND DISCRIMINATION-VERIFIED for #2214 (2026-09-07)** — in `lab-tech-encounter.spec.ts`; fails on the pre-fix bundle, passes on the fixed one, existing lab-tech test unaffected; partition re-measured to exactly one job (job1 cmd2). ⛔⛔ **It was green for four runs while proving nothing** — the syphilis tab reads "Syphilis - RPR", so exact-label steering left it positive and `syphilisTreatmentCompleted` held Next Steps pending on both code paths. See the entry for the three lessons (run against pre-fix and REQUIRE failure; two-sided tab assertions; assert the encounter did not complete).
📌 (superseded) **E2E NOT YET WRITTEN for #2214** — the entry asked for one. Regression coverage already exists (the full initial-encounter specs drive `completeNextSteps`, whose `medicationDistribution` sub-task would hang if the fix made the task uncompletable). What is missing is a positive reproduction: a recurrent encounter where Next Steps is offered with an EMPTY required set, saved, and a medication then becomes required. The tractable shape is **low hemoglobin → `DiagnosisModerateAnemiaRecurrentPhase`** (opens Next Steps, requires no medication) **+ partner HIV positive** (adds TDF3TC once the follow-ups are answered) — `completeLabResults` fills every numeric input with `12`, so it needs an option to enter an anemic hemoglobin. Verifying it borrows the single-instance app; ⛔ ask the user first.

**#2224 (B-319, tier 1, issue #2223) opened 2026-09-09** — a blood smear the nurse ordered at the lab reached the lab technician already answered "test will NOT be performed today", with the nurse's reason for not running the RDT ticked under Why not, 2/2 complete and no result field; one Save closed the order empty. Fixed in `malariaResultFormWithDefault` (not in the shared `resolveRunConfirmedByLabTechFromValue`, which ten tests reach through an extensible record that has no `bloodSmearResult`). ✅ **Discrimination-verified BOTH WAYS against the running app on 2026-09-09** — the main tree was borrowed with the user's approval and returned to `develop`: 5 new `Measurement/Test.elm` cases, one failing on pre-fix `Utils.elm`; and the e2e folded into the existing `lab-tech-encounter.spec.ts` test **fails** on the pre-fix bundle (`.form-input.yes-no.test-performed input.checked` expected 0, received 1) and passes on the fixed one, with the compiled function read out of the served `Main.js` before each run. Not yet reviewed.
**Reviewed 2026-09-09 — three findings, all fixed in `4b9948d18`, all threads answered.** ⭐ **The headline finding's suggested fix was WRONG and only running it showed that:** `laboratoryResultTaskCompleted`'s malaria branch omits the `testNotPerformedByWhyNotAtExecutionNote` clause the shared `testResultsCompleted` gives every other test (a declined malaria test leaves the activity unclosable) — but adding that clause alone also matches the PENDING order, reporting an unread smear complete. The landed fix returns `False` for `BloodSmearPendingInput` first; three cases in `Pages/Prenatal/RecurrentActivity/Test.elm` separate the three candidates and both failure modes were run. ⛔ **A review's suggested fix is a hypothesis, not a patch — write the test that separates the candidates and run the reviewer's version too.** ⚠ Also learned: Elm's `customButton` renders a disabled Save with a `disabled` CLASS and no `onClick`, not the attribute, so Playwright clicks it and counts the tab as saved; scope save locators to `:not(.disabled)`. 📌 **Nobody has opened this in a browser** — every claim about what the lab tech sees is read off the view code and locator counts.
**Round 2 (2026-09-09) found a HIGH regression the PR itself would have shipped — fixed in `5a044bf12`.** A lab tech answering "Yes" rewrites the note to `TestNoteRunConfirmedByLabTech`, and `positiveMalariaTest`'s smear branch demanded one of the NURSE's why-not notes ⇒ their positive smear diagnosed nothing, and through the INVERTED condition sent a smear-positive anemic patient to plain anemia. `positiveMalariaTest` drives five diagnoses. Fix: branch 2 tests the smear result alone, per **#631** (*"they need a different way to diagnose malaria"*) — the note list was a Feb-2023 mechanism that **#1031** silently invalidated 14 months later. Live: 9 positive smears all under why-not notes (all diagnosed), lab-tech "Yes" path in use (10 records) but `run-confirmed-by-lab-tech` + `+` = 0, so it has never fired. ➕ Issue **#2225** filed for the third finding (nurse asked for a RAPID-TEST result on a test nobody performed — pre-existing, needs triage).
**B-338 — third build `a080d98fc` on PR #2228 (2026-09-09), option (B): a NEW FIELD `field_blood_smear_ordered`, not a new enum value.** Builds 1 (note-based guard) and 2 (`BloodSmearNotRunAtLab` variant) were both rejected by review — build 2 by the rollout constraint now in memory (`enum-variant-breaks-deployed-clients`). Elm gates + phpcs green, 3134 tests, discrimination both ways; ⭐ the first confirming e2e FAILED on the lab tech's DOWNLOAD — a `list_boolean` renders as the string `"1"` and needs the `(bool)` cast in `postExecuteQueryForViewWithDbSelect` (finding 4's mechanism, on the new client, from my own field); fixed in `146342e95`, e2e ✓ 2.1 m, record left as `rdt=NULL`/`ordered=1`. **Round 3 (8 findings, all valid) fixed in `a76a00391`** — two HIGH: F1 was round 2's finding 1 returning through the pass-through I left behind (⛔ symptom fixed twice before the mechanism), F2 a mixed-fleet diagnosis hole (an old client's PATCH omits the key, so `ordered=1` survives while it writes `test_result`; verified at `HedleyRestfulSync.class.php:429`). E2E green at that head after a `login()`/`#pincode` HARNESS flake on the first try. **Round 4 (2 findings) fixed in `9977b86af`** — ⭐ its HIGH was introduced BY the round-3 fix: sharing one derivation, I then wrote a third one into the diagnosis guard, so a legacy record (flag False, smear result set) with a stray positive RDT result still diagnosed. ⛔⛔ **Session shape, three times: symptom fixed, mechanism left.** Final head `9977b86af`: 3143 tests, all gates clean, e2e green, `rdt=NULL`/`ordered=1`. Ten findings over four rounds, all fixed and answered. ➕ **E2E gap closed in `516b7e5cd`**: a second lab-tech test walks a smear the lab reads **+** through to the `malaria-recurrent` diagnosis, and re-saves it from the nurse's form — ⭐ discrimination proven (reverting `positiveMalariaTest` to develop's note-list gives `["none"]`), which is the regression BOTH the old e2e and the manual QA missed by only ever reading a NEGATIVE smear. ⛔ Three corrections while writing it: diagnoses need Danger Signs answered; the **First** button resumes an open encounter; and the smear question is not on the nurse's screen after the lab confirms the run (found by opening the screenshot). Stacked on #2224. ✅ **CI FULLY GREEN on `448f0f3b1`** — the prior red was the *Install Playwright browsers* step dying on a Google Chrome apt `Hash Sum mismatch` before any test ran; retriggered with an empty commit. ⚠ **#2229 (B-227, parallel session) is stacked on this branch at `146342e95`, three commits behind**, so it still carries the pre-round-3/4 code including both HIGHs — it inherits the fixes on rebase, but reviewing it before then reads superseded code.
➕ **B-338 added 2026-09-09 (tier 1, issue #2226):** a blood smear DECLINED and then corrected is recorded as a RAPID TEST. `bloodSmearTaken` is recomputed from the saved value alone and `BloodSmearNotTaken` is not in its list, so the decline erases "a smear was ordered", the correction is offered `standardTestResultInputsAndTasks`, and the stored `testResult` + `TestNoteRunConfirmedByLabTech` satisfies branch 1 of `positiveMalariaTest`. ⭐ **Already live, and found only by querying: ihangane has 9 records with note `run-confirmed-by-lab-tech` + smear `not-taken` + `field_test_result = negative`** — the signature exactly — plus 1 with both set; **0 on vhw / tip-somalia / uvl.** All negative, so nothing misdiagnosed, but ten records claim a rapid test was run that was not. ⛔ **B-319 produced the first step of every one**: until #2224 lands the tab opens pre-answered "No", so the decline was ACCIDENTAL. Stacks on #2224 (both edit `malariaResultFormWithDefault`). ⚠ Ten ihangane records carry a false `field_test_result`; cleanup is a separate user decision.

**Round 3 (2026-09-09): four findings, ALL pre-existing — nothing wrong with the PR.** Converted to backlog rather than grown into the branch: ➕ **#2226 (tier 1, wrong diagnosis)** a declined-then-corrected smear is recorded as a RAPID TEST (`bloodSmearTaken` is recomputed from the saved value and `BloodSmearNotTaken` is not in its list, so the decline destroys the "smear was ordered" fact) ⇒ malaria diagnosed from a test nobody ran; identical before and after this PR. ➕ **#2227** the progress report'"'"'s smear row demands `bloodSmearResultNotSet`, so **no read smear has EVER appeared**, nurse-entered included — inverted against **#632**; ⭐ the reviewer blamed the lab-tech note, and reading the issue showed the defect is wider and pre-existing. #2225 got the "green in the bar, dead in the form" detail appended. ⛔ **Stopping signal: rounds 1→2 each found a real PR defect, round 3 found only pre-existing neighbours.**
⛔⛔ **Two lessons, both expensive:** (1) **round 1 could not have found this** — the round-1 fix pulled `RecurrentActivity/Utils.elm` into the diff, and chasing IT led to the real defect one file further out. A second round after a review fix that adds a file is not ceremony. (2) **the manual QA passed and proved nothing here** — it used a NEGATIVE smear, as does the e2e; the defect only shows on a positive one.
⛔ **Two near-misses worth carrying:** (1) `completeLabResults` clicks "Yes" on `.form-input.yes-no.test-performed` for every tab, which *corrects* the pre-answered No on pre-fix code too — so the backend assertions are guards and the real discriminator is the block that reads the tab BEFORE the helper touches it. (2) That block first asserted `label.active`; `viewCustomBoolInput` puts the marker on the `input` (`classList [ ( "checked", isChecked ) ]`), so it would have been count-0 on both paths. ⭐ `Measurement/Utils` IS in the elm-test module graph (`Measurement/Test.elm` exists), so form-hydration logic here needs no seam.

**#2218 (B-324, tier 1, issue #2217) opened 2026-09-08** — the COVID path's >2-week-cough exit gated on the Tuberculosis Management feature (user chose the two-line fix over hoisting the TB check). ⚠ **Pushed as the tests-only commit `2754512ea` FIRST** (user chose CI over borrowing the main tree for the e2e discrimination run); the fix `2cbd4b708` sits in the worktree and is pushed once `e2e_playwright_1` has failed on record — e2e jobs require only the lint jobs, so the red `test_elm` on that commit does not block them. Not yet reviewed. ⭐ CI runs with the TB flag OFF (nothing in the install or CI sets it); local ddev sets it ON. tip-somalia, the only TB-off site, has zero acute illness encounters.
...
📌 Tier 1 READY after B-392 shipped (PR #2273): **none** (B-421 shipped 2026-09-22 as PR #2275, B-422 2026-09-23 as PR #2283). ⭐ **Tier 2 is not empty any more** — the 2026-09-01 note below saying so is superseded. R28 refilled it: **B-195** (half on hold as #2156, ⛔ do not re-pitch), **B-308**, **B-311**, **B-317**. Tier 1 READY after B-240 shipped: **B-286, B-288, B-289, B-319, B-320, B-323** — the last two cluster with this one and with B-237 on the same prenatal HIV surface.
...
| #2193 | #2192 | **B-307** | `B-307-followup-encounter-same-day-guard` | **fully green** (all 10 checks incl. simpletest + 3 e2e), review done, all 5 threads closed. ⚠ Its review found a **HIGH REGRESSION THIS PR HAD INTRODUCED** (Acute Illness allows same-day subsequent encounters by design) plus two CHW/nurse-split misses — all fixed `d10156393`; the dead-tap dialog folded in `dc5db4d87` at the user's direction; the unloaded-data finding judged a NON-ISSUE by the user. ⛔ Lesson: **each program has its own same-day rule — read every participant page, do not generalise from one** |
| #2191 | #2190 | **B-232** | `B-232-immunization-followup-pane` | **fully green** (all 10 checks incl. simpletest + 3 e2e), review done — no correctness findings; its two LOW findings **LEFT AS IS by the user** (*"leave #2191 as is and file a new item"*), both threads answered and closed: (1) same-day second Well Child encounter never clears the entry ➔ **split out as B-307**, (2) the pane's own `limitDate` line is untested — ⚠ the reviewer's suggested remedy does NOT close it, only a shared `followUpsLimitDate` helper across all six panes would. ⚠ **Rebased onto `develop` 2026-09-03** after #2193/#2180/#2183 merged: `View.elm` auto-merged, `Test.elm` conflicted twice (both sides extended the same imports, and each added a whole test function at the same point) — resolved as the union, with `generateImmunizationFollowUpEntriesTests` placed at its alphabetical position. Force-pushed `5273f45f5`, **all 10 checks green again**, MERGEABLE/CLEAN. |

Worktrees held, to be removed **when their PR merges**: `ihangane-wt/{B-272-vitals-previous-reading, B-303-B-304-translate-corrections, B-280-subsequent-diagnosis-correction, B-299-village-stock-cache, B-213-scorecard-age-at-measurement, B-232-immunization-followup-pane, B-307-followup-encounter-same-day-guard}`.
...
- ✅ **PR #2239 (B-336 re-land) MERGED to `develop` 2026-09-14 07:39 UTC** — `763e74bf4` is now an ancestor of `origin/develop`; B-336 is IMPLEMENTED and is in release #2128.
...
- ✅ **B-336 re-landed on develop via #2239 (2026-09-14 07:39 UTC).** (Historical:) ⛔⛔ **B-336 was NOT on develop** (see the correction at the top of "Open right now"). Land it before the release is cut.
- **Before release #2128 is cut, three findings bear on it:** B-336 (stranded — ✅ landed via #2239), **B-339** (✅ PR #2241) and **B-342** (🅿 PARKED by user 2026-09-15 — measured rare, see entry) (tier-1 defects introduced by #2216 and #2228, both un-deployed), and **B-341** (add the photo-repair run to #2128's post-deploy steps, plus the beverages re-run reminder).
- Tier 1 also: **B-343** (live — 3 ihangane records, ✅ PR #2245), **B-344** (✅ PR #2253), **B-345** — all on the prenatal HIV surface; B-345 is best built with B-336.
...
- **B-325** 🅿 user, 2026-09-23: *"park"* — after a live estimate of ~1–3 lost uploads/year on ihangane. Revisit only if per-record commit is revived.
