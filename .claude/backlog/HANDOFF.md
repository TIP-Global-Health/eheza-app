...
🟡 **PR #2289 open 2026-09-24 (B-216, tier 2, issue #2288)** — a birth-date change moves group enrolment/graduation dates only where the person is the child; an adult's correction no longer rewrites their children's. One PHP file + scenario 4 in `HedleyStatsCalculation` (before B-248's early return). Test `cc9771a82` + fix `2757fde08` (user: push the fix without waiting for a fail-first run). Worktree released. CI pending. Live damage: ihangane 560, vhw 22, none current; backfill not done (user: code fix only).
