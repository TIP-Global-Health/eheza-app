...
🟡 **PR #2269 open 2026-09-17 (B-259 + B-222, tier 4, issues #2267 / #2268)** — person form: entering the create or edit page resets the app-wide person save request, so a fresh form no longer opens with "Person has been saved" (or an old error); a person PATCH stores its real result, so a failed edit shows the backend error. Three Elm files; elm make, elm-review, elm-test green locally; no new test. Worktree released. **Review round 1 (medium, 2026-09-17): 2 findings posted inline** — children's address PATCHes share `postPerson` and navigate (hides the parent's failure); WhatsApp PATCH failure lingers for the contact form. Proposed: only the edited person's own PATCH writes `postPerson` (new initiator for child saves). **Awaiting the user's decision.**
