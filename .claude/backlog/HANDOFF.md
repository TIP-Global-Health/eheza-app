...
🟡 **PR #2243 open 2026-09-14 (B-364, tier 1, issue #2242)** — the Prenatal twin: `generatePreviousMeasurements` keeps encounters started on or before the assembled encounter's day (same-day kept). 6 elm-test cases, 4 fail pre-fix, the strict-`LT` variant fails the 2 same-day ones. Also covers B-363(d). Deployed defect, latent. **Reviewed 2026-09-14: 1 finding** (report opened from the older encounter hides later visits, https://github.com/TIP-Global-Health/eheza-app/pull/2243#discussion_r4003534984) — **awaiting the user's decision** on (a) that finding (#2241's answer was to keep it) and (b) switching `/= GT` to strict `LT` now that same-day pairs are shown to be duplicates.
