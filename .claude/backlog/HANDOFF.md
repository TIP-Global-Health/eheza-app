...
🟡 **PR #2283 open 2026-09-23 (B-422, tier 1, issue #2282)** — HIV program Diagnostics: the result's dirty flag survives hydration; a result is saved only with a test run; the close-as-not-diagnosed check reads the saved value; repeated taps on "diagnosed positive" / "test run" leave the form alone. Four Elm files incl. the first HIV test file (7 cases through the real `update`). Worktree released. **Review not yet requested.**
