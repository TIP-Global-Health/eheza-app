...
🟡 **PR #2275 open 2026-09-22 (B-421, tier 1, issue #2274)** — NCD Pregnancy/HIV test forms clear the result, as dirty, when the run is withdrawn ("performed" → No, or known as positive); the two NCD result setters mark it dirty; `patientIsPregnantAtEncounter` takes a result only from a run that produced it (covers saved records and old-build devices). Six Elm files (4 source + 2 test), 8 elm-tests, 5 fail with the fix stubbed; elm make / elm-review / comment check green locally. Worktree released. **Review not yet requested.** Sibling B-422 (HIV Diagnostics `testResultDirty = False`) is still READY.
