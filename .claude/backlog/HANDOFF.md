...
⚠ **For #2241's review:** its strict-`LT` cut also drops SAME-DAY encounters from each other's history, including from the newest one, and "only possible across offline devices" is not true: live ihangane has **217 same-day NCD encounter pairs over 42 participants** (mostly empty duplicates — B-195's class). Prenatal has 4,127 same-day nurse pairs, 66 with data on both sides, which is why #2243 cuts later DAYS only.

🟡 **PR #2243 open 2026-09-14 (B-364, tier 1, issue #2242)** — the Prenatal twin: `generatePreviousMeasurements` keeps encounters started on or before the assembled encounter's day (same-day kept). 6 elm-test cases, 4 fail pre-fix, the strict-`LT` variant fails the 2 same-day ones. Also covers B-363(d). Deployed defect, latent. Awaits CI + `/code-review medium B-364-prenatal-history-before-encounter`.
